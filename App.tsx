...
import type { Page, Student, Instructor, Course, Transaction, Staff, Exam, Classroom, TrainingField, Candidate, AdminProfile, NotificationLog, NotificationTemplate } from './types';
import { 
  navigationItems, 
  ...
  MenuIcon, SunIcon, MoonIcon, UploadCloudIcon, BarChartIcon, UserCircleIcon, TriangleAlertIcon,
  SendIcon, MessageCircleIcon, CheckCircleIcon, XCircleIcon, ClockIcon, FolderKanbanIcon, ClipboardUserIcon, BriefcaseIcon, CalendarDaysIcon, SchoolIcon, CircleDollarSignIcon
} from './constants';
...
const initialCourses: Course[] = [
    { name: 'Advanced JavaScript', code: 'JS-ADV-01', instructor: 'Nguyễn Văn A', students: { enrolled: 27, capacity: 30 }, status: 'Ongoing', schedule: 'Mon, Wed, Fri' },
...
const initialNotifications: NotificationLog[] = [
  { id: 'NOT001', title: 'Nhắc lịch thi Lớp Java K11', message: 'Lịch thi vào ngày 25/12/2025 lúc 20h33 tại phòng A101.', recipient: 'Lớp Java K11', channels: ['Zalo'], status: 'Success', sentTime: '2025-12-24 20:33', type: 'Exam', templateId: 'TMP002' },
  { id: 'NOT002', title: 'Thông báo học phí Lớp Web K12', message: 'Vui lòng đóng học phí trước ngày 01/01/2026.', recipient: 'Nguyễn Văn A', channels: ['Telegram'], status: 'Failed', sentTime: '2025-12-24 19:10', type: 'Tuition', templateId: 'TMP003' },
  { id: 'NOT003', title: 'Nghỉ lễ Quốc Khánh', message: 'Trung tâm nghỉ lễ 02/09, chúc các bạn nghỉ lễ vui vẻ.', recipient: 'Toàn trường', channels: ['Email', 'Zalo'], status: 'Success', sentTime: '2025-09-01 08:00', type: 'General' },
];

const initialTemplates: NotificationTemplate[] = [
  { id: 'TMP001', name: 'Nhắc lịch học', content: 'Nhắc lịch học: {ten_lop} - {ngay_hoc} - {gio_hoc} - {phong}', type: 'Schedule' },
  { id: 'TMP002', name: 'Nhắc lịch thi', content: 'Thông báo: Kỳ thi {mon_thi} sẽ diễn ra vào {ngay_thi} lúc {gio_thi} tại {dia_diem}.', type: 'Exam' },
  { id: 'TMP003', name: 'Nhắc học phí', content: 'Chào {hoc_vien}, vui lòng hoàn thành học phí khóa {khoa_hoc} số tiền {so_tien} trước ngày {han_chot}.', type: 'Tuition' },
];
...
const Modal: React.FC<{ isOpen: boolean; onClose: () => void; title: string; children: React.ReactNode; size?: 'md' | 'lg' | 'xl' | '2xl' }> = ({ isOpen, onClose, title, children, size = '2xl' }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4" onClick={onClose}>
            <div className={`bg-light-bg-card dark:bg-dark-bg-card rounded-lg shadow-xl w-full ${sizeClasses[size]} transform transition-all max-h-[90vh] overflow-y-auto`} onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center p-5 border-b border-light-border dark:border-dark-border sticky top-0 bg-light-bg-card dark:bg-dark-bg-card z-10">
...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
    const statusMap: { [key: string]: string } = {
        ...
        'Success': 'bg-status-green-bg-light dark:bg-status-green-bg-dark text-status-green',
        'Failed': 'bg-status-red-bg-light dark:bg-status-red-bg-dark text-status-red',
        'Pending': 'bg-status-yellow-bg-light dark:bg-status-yellow-bg-dark text-status-yellow',
    };
...
const NotificationCenter: React.FC<{ notifications: NotificationLog[], onNavigate: (page: Page) => void }> = ({ notifications, onNavigate }) => {
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            <button 
                ...
                className="p-2 text-light-text-muted dark:text-dark-text-muted hover:text-light-text-main dark:hover:text-dark-text-main relative"
            >
                <BellIcon className="w-6 h-6" />
                {notifications.some(n => n.status === 'Failed') && (
                     <span className="absolute top-1 right-1 w-2.5 h-2.5 bg-red-500 rounded-full border-2 border-light-bg-main dark:border-dark-bg-main"></span>
                )}
            </button>

            {isOpen && (
                <div className="absolute right-0 mt-2 w-80 bg-light-bg-card dark:bg-dark-bg-card rounded-lg shadow-xl border border-light-border dark:border-dark-border z-50">
                    <div className="p-4 border-b border-light-border dark:border-dark-border flex justify-between items-center">
                        ...
                        <button onClick={() => { onNavigate('NotificationManagement'); setIsOpen(false); }} className="text-xs text-primary-blue hover:underline">Xem tất cả</button>
                    </div>
                    <div className="max-h-80 overflow-y-auto">
                        {recentNotifications.length === 0 ? (
                            <p className="p-4 text-center text-sm text-light-text-muted">Không có thông báo nào.</p>
                        ) : (
                            <ul>
                                {recentNotifications.map(notif => (
                                    <li key={notif.id} className="p-4 border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                        <div className="flex items-start gap-3">
                                            {notif.status === 'Success' ? <CheckCircleIcon className="w-5 h-5 text-green-500 mt-0.5" /> : 
                                             notif.status === 'Failed' ? <XCircleIcon className="w-5 h-5 text-red-500 mt-0.5" /> :
                                             <BellIcon className="w-5 h-5 text-yellow-500 mt-0.5" />}
                                            <div>
                                                <p className="text-sm font-medium text-light-text-main dark:text-dark-text-main">{notif.title}</p>
                                                <p className="text-xs text-light-text-muted dark:text-dark-text-muted mt-1">{notif.sentTime} • {notif.channels.join(', ')}</p>
                                            </div>
                                        </div>
                                    </li>
                                ))}
...
const SendNotificationModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    initialData?: Partial<NotificationLog>;
    templates: NotificationTemplate[];
    onSend: (notification: NotificationLog) => void;
}> = ({ isOpen, onClose, initialData, templates, onSend }) => {
    const [title, setTitle] = useState('');
    const [message, setMessage] = useState('');
    const [recipient, setRecipient] = useState('');
    const [channels, setChannels] = useState<('Zalo' | 'Email' | 'Telegram')[]>(['Zalo']);
    ...
    useEffect(() => {
        if (isOpen && initialData) {
            ...
            if (initialData.channels) setChannels(initialData.channels);
             ...
             setTitle(''); setMessage(''); setRecipient(''); setChannels(['Zalo']); setScheduleTime(''); setSelectedTemplate('');
        }
    }, [isOpen, initialData]);
    ...
    const toggleChannel = (channel: 'Zalo' | 'Email' | 'Telegram') => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        onSend({
            ...
            recipient,
            channels,
            status: scheduleTime ? 'Pending' : (Math.random() > 0.2 ? 'Success' : 'Failed'), // Mock random status
            sentTime: scheduleTime || new Date().toLocaleString(),
            type: initialData?.type || 'General',
            templateId: selectedTemplate || undefined, // Save the template ID
        });
        onClose();
    };

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Gửi Thông Báo">
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                    ...
                    <select value={selectedTemplate} onChange={handleTemplateChange} className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue">
                ...
                <div>
                    ...
                    <input type="text" value={title} onChange={e => setTitle(e.target.value)} placeholder="Nhập tiêu đề..." required className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue" />
                </div>
                 <div>
                    <label className="block text-sm font-medium mb-2">Nội dung</label>
                    <textarea value={message} onChange={e => setMessage(e.target.value)} placeholder="Nhập nội dung thông báo..." required className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue h-32"></textarea>
                </div>
                <div>
                    <label className="block text-sm font-medium mb-2">Kênh gửi</label>
                    <div className="flex space-x-4">
                        {['Zalo', 'Telegram', 'Email'].map(c => (
                            <label key={c} className="flex items-center space-x-2 cursor-pointer">
                                <input type="checkbox" checked={channels.includes(c as any)} onChange={() => toggleChannel(c as any)} className="rounded text-primary-blue focus:ring-primary-blue" />
                                <span>{c}</span>
                            </label>
                        ))}
                    </div>
                ...
                <div>
                     <label className="block text-sm font-medium mb-2">Hẹn giờ (Để trống nếu gửi ngay)</label>
                     <input type="datetime-local" value={scheduleTime} onChange={e => setScheduleTime(e.target.value)} className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue text-light-text-muted dark:text-dark-text-muted" />
                </div>
                <div className="flex justify-end space-x-4 pt-4">
                    ...
                    <button type="submit" className="bg-primary-blue hover:bg-primary-blue-hover text-white px-6 py-2 rounded-lg flex items-center">
                         <SendIcon className="w-4 h-4 mr-2" /> {scheduleTime ? 'Lên lịch' : 'Gửi ngay'}
                    </button>
                </div>
            </form>
        </Modal>
...
const AddEditTemplateForm: React.FC<{
    template: NotificationTemplate | null;
    onSubmit: (formData: Omit<NotificationTemplate, 'id'>) => void;
    onClose: () => void;
    ...
    const placeholders = ['{ten_lop}', '{ngay_hoc}', '{gio_hoc}', '{phong}', '{hoc_vien}', '{so_tien}', '{han_chot}', '{mon_thi}', '{ngay_thi}', '{gio_thi}', '{dia_diem}'];
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        onSubmit({ name, content, type: type as any });
    };

    return (
        <form onSubmit={handleSubmit} className="space-y-4">
            <div>
                <label className="block text-sm font-medium mb-2">Tên mẫu</label>
                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Ví dụ: Nhắc lịch học" required className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue" />
            </div>
             <div>
                <label className="block text-sm font-medium mb-2">Loại thông báo</label>
                <select value={type} onChange={e => setType(e.target.value as any)} required className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue">
            ...
            <div>
                <label className="block text-sm font-medium mb-2">Nội dung mẫu</label>
                <textarea value={content} onChange={e => setContent(e.target.value)} placeholder="Nhập nội dung..." required className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue h-32"></textarea>
                <div className="mt-2">
                    <p className="text-xs text-light-text-muted dark:text-dark-text-muted mb-1">Gợi ý biến (click để thêm):</p>
                    <div className="flex flex-wrap gap-2">
                        {placeholders.map(ph => (
                            <button key={ph} type="button" onClick={() => insertPlaceholder(ph)} className="text-xs bg-light-bg-nav dark:bg-dark-bg-nav hover:bg-primary-blue hover:text-white px-2 py-1 rounded transition-colors">
                                {ph}
                            </button>
                        ))}
                    </div>
            ...
            <div className="flex justify-end space-x-4 pt-4">
                <button type="button" onClick={onClose} className="bg-light-bg-nav dark:bg-dark-bg-nav hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover px-6 py-2 rounded-lg">Hủy</button>
                <button type="submit" className="bg-primary-blue hover:bg-primary-blue-hover text-white px-6 py-2 rounded-lg">Lưu Mẫu</button>
            </div>
        </form>
    );
};

const TemplateHistoryModal: React.FC<{
    isOpen: boolean;
    onClose: () => void;
    template: NotificationTemplate | null;
    notifications: NotificationLog[];
}> = ({ isOpen, onClose, template, notifications }) => {
    const history = notifications.filter(n => n.templateId === template?.id);

    return (
        <Modal isOpen={isOpen} onClose={onClose} title="Lịch sử sử dụng mẫu" size="xl">
            <div className="mb-4">
                 <p className="text-sm text-light-text-muted dark:text-dark-text-muted">Mẫu: <span className="font-semibold text-light-text-main dark:text-dark-text-main">{template?.name}</span></p>
            </div>
            {history.length > 0 ? (
                <div className="overflow-x-auto bg-light-bg-card dark:bg-dark-bg-card rounded-lg border border-light-border dark:border-dark-border">
                    <table className="w-full text-left">
                        <thead className="bg-light-bg-nav dark:bg-dark-bg-nav border-b border-light-border dark:border-dark-border">
                            <tr>
                                <th className="p-3 text-xs font-semibold uppercase text-light-text-muted dark:text-dark-text-muted">Thời gian</th>
                                <th className="p-3 text-xs font-semibold uppercase text-light-text-muted dark:text-dark-text-muted">Người nhận</th>
                                <th className="p-3 text-xs font-semibold uppercase text-light-text-muted dark:text-dark-text-muted">Tiêu đề</th>
                                <th className="p-3 text-xs font-semibold uppercase text-light-text-muted dark:text-dark-text-muted">Kênh</th>
                                <th className="p-3 text-xs font-semibold uppercase text-light-text-muted dark:text-dark-text-muted">Trạng thái</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-light-border dark:divide-dark-border">
                            {history.map(log => (
                                <tr key={log.id} className="hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                    <td className="p-3 text-sm">{log.sentTime}</td>
                                    <td className="p-3 text-sm font-medium">{log.recipient}</td>
                                    <td className="p-3 text-sm truncate max-w-xs">{log.title}</td>
                                    <td className="p-3 text-sm flex gap-1">
                                        {log.channels.map(c => <span key={c} className="bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-xs">{c}</span>)}
                                    </td>
                ...
                <div className="text-center py-8 text-light-text-muted dark:text-dark-text-muted bg-light-bg-nav dark:bg-dark-bg-nav rounded-lg border-2 border-dashed border-light-border dark:border-dark-border">
                    <ClockIcon className="w-8 h-8 mx-auto mb-2 opacity-50" />
                    <p>Chưa có thông báo nào sử dụng mẫu này.</p>
                </div>
            )}
             <div className="flex justify-end mt-6">
                <button onClick={onClose} className="bg-primary-blue text-white px-4 py-2 rounded-lg hover:bg-primary-blue-hover transition-colors">Đóng</button>
            </div>
        </Modal>
    );
};
    ...
    return (
        <aside className={`w-64 bg-light-bg-nav dark:bg-dark-bg-nav flex flex-col fixed h-full z-40 transform transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
            ...
            <div className="px-4 py-4 border-t border-light-border dark:border-dark-border space-y-2">
                 <a href="#" onClick={(e) => { e.preventDefault(); handleNav('NotificationTemplates'); }} className={`flex items-center px-4 py-2.5 text-sm rounded-lg transition-colors ${activePage === 'NotificationTemplates' ? 'bg-primary-blue text-white font-semibold' : 'text-light-text-muted dark:text-dark-text-muted hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover hover:text-light-text-main dark:hover:text-dark-text-main'}`}>
                    <MessageCircleIcon className="w-5 h-5 mr-3" />
                    Mẫu thông báo
                </a>
                 <a href="#" onClick={(e) => { e.preventDefault(); handleNav('Settings'); }} className={`flex items-center px-4 py-2.5 text-sm rounded-lg transition-colors ${activePage === 'Settings' ? 'bg-primary-blue text-white font-semibold' : 'text-light-text-muted dark:text-dark-text-muted hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover hover:text-light-text-main dark:hover:text-dark-text-main'}`}>
...
const AppLayout: React.FC<{ children: React.ReactNode, activePage: Page; onNavigate: (page: Page) => void, theme: Theme, setTheme: (theme: Theme) => void, adminProfile: AdminProfile, notifications: NotificationLog[] }> = ({ children, activePage, onNavigate, theme, setTheme, adminProfile, notifications }) => {
    const [isSidebarOpen, setIsSidebarOpen] = useState(false);
    const pageTitle = useMemo(() => {
        if (activePage === 'Profile') return 'Hồ sơ';
        if (activePage === 'Reports') return 'Báo cáo';
        if (activePage === 'NotificationManagement') return 'Quản lý thông báo';
        if (activePage === 'NotificationHistory') return 'Lịch sử tin nhắn';
        if (activePage === 'NotificationTemplates') return 'Mẫu thông báo';
        if (activePage === 'Settings') return 'Cài đặt';
            ...
            <div className="flex-1 flex flex-col lg:ml-64 h-screen overflow-hidden">
                <header className="flex-shrink-0 bg-light-bg-main/80 dark:bg-dark-bg-main/80 backdrop-blur-sm z-20 flex items-center justify-between h-16 px-4 md:px-8 border-b border-light-border dark:border-dark-border">
                    ...
                    <div className="flex items-center space-x-4">
                        ...
                        <NotificationCenter notifications={notifications} onNavigate={onNavigate} />
...
// PAGE COMPONENTS
const DashboardPage: React.FC<{ theme: Theme, notifications: NotificationLog[] }> = ({ theme, notifications }) => {
    ...
    const failCount = notifications.filter(n => n.status === 'Failed' || n.status === 'Pending').length;
    ...
    return (
        <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <StatCard title="Tin nhắn đã gửi (tháng)" value={totalSent.toString()} icon={<SendIcon className="w-6 h-6 text-primary-blue" />} />
                <StatCard title="Tỉ lệ gửi thành công" value={`${successRate}%`} icon={<CheckCircleIcon className="w-6 h-6 text-status-green" />} />
                <StatCard title="Thất bại / Chờ xử lý" value={failCount.toString()} icon={<TriangleAlertIcon className="w-6 h-6 text-status-red" />} />
            </div>
...
const NotificationTemplatesPage: React.FC<{ 
    ...
    notifications: NotificationLog[];
}> = ({ templates, setTemplates, notifications }) => {
    ...
    }>({ view: false, add: false, edit: false, delete: false, history: false, selected: null });
    ...
    const handleAddTemplate = (data: Omit<NotificationTemplate, 'id'>) => {
        const newTemplate = { ...data, id: `TMP${Date.now()}` };
        setTemplates([...templates, newTemplate]);
        setModalState({ ...modalState, add: false });
    };

    const handleUpdateTemplate = (data: Omit<NotificationTemplate, 'id'>) => {
        if (!modalState.selected) return;
        setTemplates(templates.map(t => t.id === modalState.selected!.id ? { ...t, ...data } : t));
        setModalState({ ...modalState, edit: false, selected: null });
    };

    const handleDeleteTemplate = () => {
         if (!modalState.selected) return;
         setTemplates(templates.filter(t => t.id !== modalState.selected!.id));
         setModalState({ ...modalState, delete: false, selected: null });
    };

    // Check if template is used
    const isTemplateUsed = (templateId: string) => {
        // Simple check against logs, in a real app you might check against scheduled tasks or active courses configs
        return notifications.some(n => n.templateId === templateId);
    };

    return (
        <div>
            <h1 className="text-3xl font-bold mb-2">Quản lý Mẫu Thông Báo</h1>
            <p className="text-light-text-muted dark:text-dark-text-muted mb-6">Tạo và chỉnh sửa các mẫu tin nhắn để gửi nhanh.</p>

            <Modal isOpen={modalState.add || modalState.edit} onClose={() => setModalState({...modalState, add: false, edit: false, selected: null})} title={modalState.add ? "Tạo Mẫu Mới" : "Chỉnh sửa Mẫu"}>
                <AddEditTemplateForm 
                    template={modalState.selected}
                    onSubmit={modalState.add ? handleAddTemplate : handleUpdateTemplate}
                    onClose={() => setModalState({...modalState, add: false, edit: false, selected: null})}
                />
            </Modal>

            <Modal isOpen={modalState.delete} onClose={() => setModalState({...modalState, delete: false, selected: null})} title="Xóa Mẫu Thông Báo" size="md">
                 {modalState.selected && isTemplateUsed(modalState.selected.id) ? (
                     <div>
                         <p className="text-red-500 mb-4 flex items-center"><TriangleAlertIcon className="w-5 h-5 mr-2"/> Không thể xóa mẫu này!</p>
                         <p>Mẫu <strong>{modalState.selected.name}</strong> đang được sử dụng trong lịch sử thông báo hoặc đang được gán cho lớp học.</p>
                         <div className="flex justify-end mt-6">
                            <button onClick={() => setModalState({...modalState, delete: false, selected: null})} className="bg-primary-blue text-white px-4 py-2 rounded-lg">Đã hiểu</button>
                         </div>
                     </div>
                 ) : (
                    <div>
                        <p>Bạn có chắc chắn muốn xóa mẫu <strong>{modalState.selected?.name}</strong>? Hành động này không thể hoàn tác.</p>
                        <div className="flex justify-end space-x-4 mt-6">
                            <button onClick={() => setModalState({...modalState, delete: false, selected: null})} className="bg-gray-200 dark:bg-gray-700 px-4 py-2 rounded-lg">Hủy</button>
                            <button onClick={handleDeleteTemplate} className="bg-red-500 text-white px-4 py-2 rounded-lg hover:bg-red-600">Xóa</button>
                        </div>
                    </div>
                 )}
            </Modal>
            ...
            <TemplateHistoryModal 
                ...
                onClose={() => setModalState({...modalState, history: false, selected: null})}
                template={modalState.selected}
            ...
            <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setModalState({...modalState, add: true})} className="bg-primary-blue text-white px-4 py-2 rounded-lg flex items-center hover:bg-primary-blue-hover">
                    <PlusIcon className="w-5 h-5 mr-2" /> Tạo Mẫu Mới
                </button>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {filteredTemplates.map(template => (
                    <div key={template.id} className="bg-light-bg-card dark:bg-dark-bg-card p-6 rounded-lg border border-light-border dark:border-dark-border shadow-sm hover:shadow-md transition-shadow relative group">
                        <div className="absolute top-4 right-4 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity">
                             <button onClick={() => setModalState({...modalState, history: true, selected: template})} title="Lịch sử sử dụng" className="p-1.5 bg-gray-100 dark:bg-gray-700 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-blue-600">
                                <ClockIcon className="w-4 h-4" />
                            </button>
                            <button onClick={() => setModalState({...modalState, edit: true, selected: template})} className="p-1.5 bg-gray-100 dark:bg-gray-700 rounded hover:bg-blue-100 dark:hover:bg-blue-900 text-primary-blue">
                                <EditIcon className="w-4 h-4" />
                            </button>
                             <button onClick={() => setModalState({...modalState, delete: true, selected: template})} className="p-1.5 bg-gray-100 dark:bg-gray-700 rounded hover:bg-red-100 dark:hover:bg-red-900 text-red-500">
                                <Trash2Icon className="w-4 h-4" />
                            </button>
                        </div>
                        <div className="flex items-center justify-between mb-3">
                             <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                                 template.type === 'Schedule' ? 'bg-blue-100 text-blue-700' :
                                 template.type === 'Exam' ? 'bg-red-100 text-red-700' :
                                 template.type === 'Tuition' ? 'bg-yellow-100 text-yellow-700' :
                                 'bg-gray-100 text-gray-700'
                             }`}>
                                 {template.type === 'Schedule' ? 'Lịch học' : template.type === 'Exam' ? 'Lịch thi' : template.type === 'Tuition' ? 'Học phí' : 'Chung'}
                             </span>
                        </div>
                        <h3 className="font-semibold text-lg mb-2">{template.name}</h3>
                        <p className="text-sm text-light-text-muted dark:text-dark-text-muted line-clamp-3 h-16">
                            {template.content}
                        </p>
                        <div className="mt-4 pt-4 border-t border-light-border dark:border-dark-border flex justify-between items-center text-xs text-light-text-muted dark:text-dark-text-muted">
                            <span>ID: {template.id}</span>
                            <span>{isTemplateUsed(template.id) ? 'Đang sử dụng' : 'Chưa sử dụng'}</span>
                        </div>
                    </div>
                ))}
            </div>
...
const NotificationHistoryPage: React.FC<{
    ...
    const filteredNotifications = useMemo(() => {
        return notifications.filter(n => {
            ...
            const matchesChannel = filterChannel === 'all' || n.channels.includes(filterChannel as any);
            const matchesStatus = filterStatus === 'all' || n.status === filterStatus;
            // Assuming sentTime format is "YYYY-MM-DD HH:mm" or similar that includes date
            const matchesDate = filterDate === '' || n.sentTime.startsWith(filterDate);
            return matchesSearch && matchesChannel && matchesStatus && matchesDate;
        });
    }, [notifications, searchTerm, filterChannel, filterStatus, filterDate]);
    ...
    return (
        <div>
            <h1 className="text-3xl font-bold mb-2">Lịch sử Tin nhắn</h1>
            ...
            <div className="flex flex-wrap gap-4 mb-6 bg-light-bg-card dark:bg-dark-bg-card p-4 rounded-lg shadow-sm items-center">
                <div className="relative flex-grow min-w-[200px]">
                    ...
                    <input 
                        type="text" 
                        placeholder="Tìm theo người nhận, tiêu đề..." 
                ...
                <select 
                    className="bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
                    value={filterChannel}
                ...
                <input 
                    type="date" 
                    className="bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue text-light-text-muted dark:text-dark-text-muted"
                    value={filterDate}
                    onChange={e => { setFilterDate(e.target.value); setCurrentPage(1); }}
                />
                 <button
                    onClick={() => { setSearchTerm(''); setFilterChannel('all'); setFilterStatus('all'); setFilterDate(''); setCurrentPage(1); }}
            ...
            {/* Data Table */}
            <div className="bg-light-bg-card dark:bg-dark-bg-card rounded-lg overflow-x-auto border border-light-border dark:border-dark-border">
                <table className="w-full text-left whitespace-nowrap">
                    <thead className="bg-light-bg-nav dark:bg-dark-bg-nav border-b border-light-border dark:border-dark-border">
                        <tr>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Thời gian</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Người nhận</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Tiêu đề / Nội dung</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Kênh</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Trạng thái</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted text-right">Hành động</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-light-border dark:divide-dark-border">
                        {paginatedNotifications.map(log => (
                            <tr key={log.id} className="hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                <td className="p-4 text-sm">{log.sentTime}</td>
                                <td className="p-4 font-medium">{log.recipient}</td>
                                <td className="p-4 max-w-xs overflow-hidden">
                                    <div className="truncate font-medium text-sm">{log.title}</div>
                                    <div className="truncate text-xs text-light-text-muted dark:text-dark-text-muted">{log.message}</div>
                                </td>
                                <td className="p-4">
                                    <div className="flex space-x-1">
                                        {log.channels.map(c => (
                                            <span key={c} className={`px-2 py-0.5 text-xs rounded border ${
                                                c === 'Zalo' ? 'bg-blue-50 border-blue-200 text-blue-600' :
                                                c === 'Email' ? 'bg-yellow-50 border-yellow-200 text-yellow-600' :
                                ...
                                <td className="p-4 text-right">
                                    <button onClick={() => handleView(log)} className="text-light-text-muted dark:text-dark-text-muted hover:text-primary-blue mx-1 p-1" title="Xem chi tiết"><EyeIcon className="w-5 h-5" /></button>
                                    <button onClick={() => handleResend(log)} className="text-light-text-muted dark:text-dark-text-muted hover:text-green-600 mx-1 p-1" title="Gửi lại"><SendIcon className="w-5 h-5" /></button>
                                </td>
                            </tr>
                        ))}
                         {paginatedNotifications.length === 0 && (
                            <tr>
                                <td colSpan={6} className="p-8 text-center text-light-text-muted dark:text-dark-text-muted">
            ...
            {/* View Detail Modal */}
            <Modal isOpen={isViewModalOpen} onClose={() => setIsViewModalOpen(false)} title="Chi tiết Tin nhắn">
                {selectedLog && (
                    <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                             <div>
                                <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Người nhận</label>
                                <p className="font-medium">{selectedLog.recipient}</p>
                            </div>
                            <div>
                                <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Thời gian gửi</label>
                                <p className="font-medium">{selectedLog.sentTime}</p>
                            </div>
                            <div>
                                <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Kênh gửi</label>
                                <div className="flex gap-2 mt-1">
                                     {selectedLog.channels.map(c => <span key={c} className="px-2 py-0.5 bg-gray-100 dark:bg-gray-700 rounded text-sm">{c}</span>)}
                                </div>
                            </div>
                             <div>
                                <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Trạng thái</label>
                                <div className="mt-1"><StatusBadge status={selectedLog.status} /></div>
                            </div>
                        </div>
                        <hr className="border-light-border dark:border-dark-border"/>
                        <div>
                            <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Tiêu đề</label>
                            <p className="font-semibold text-lg">{selectedLog.title}</p>
                        </div>
                         <div>
                            <label className="text-xs text-light-text-muted dark:text-dark-text-muted uppercase">Nội dung</label>
                            <div className="bg-light-bg-nav dark:bg-dark-bg-nav p-4 rounded-lg mt-2 text-sm whitespace-pre-wrap border border-light-border dark:border-dark-border">
                                {selectedLog.message}
                            </div>
                        </div>
                        <div className="flex justify-end pt-4">
                             <button onClick={() => { setIsViewModalOpen(false); handleResend(selectedLog); }} className="bg-primary-blue text-white px-4 py-2 rounded-lg hover:bg-primary-blue-hover flex items-center">
                                <SendIcon className="w-4 h-4 mr-2" /> Gửi Lại
                            </button>
                        </div>
                    </div>
                )}
                            ...
                            <tr key={student.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                ...
                                <td className="p-4">
                                    <div className="flex justify-center space-x-2 text-light-text-muted dark:text-dark-text-muted">
                                         <button onClick={() => onOpenSendModal({ recipient: student.name, type: 'General' })} title="Gửi thông báo" className="hover:text-primary-blue p-1 rounded-md hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav"><MessageCircleIcon className="w-5 h-5" /></button>
...
const InstructorManagementPage: React.FC = () => {
    ...
    }>({ view: false, add: false, edit: false, delete: false, bulkDelete: false, selected: null });
    ...
    const handleBulkDelete = () => {
        setInstructors(instructors.filter(i => !selectedInstructors.includes(i.id)));
        setSelectedInstructors([]);
        setModalState({ ...modalState, bulkDelete: false });
    };
    ...
    const handleSelectAll = () => {
        if (selectedInstructors.length === paginatedInstructors.length) {
            setSelectedInstructors([]);
        } else {
    ...
    const clearSelectionAndSetPage = (setter: React.Dispatch<React.SetStateAction<any>>, value: any) => {
        setter(value);
        setCurrentPage(1);
        setSelectedInstructors([]);
    }
     ...
     return (
        <div>
            ...
            <Modal isOpen={modalState.delete} onClose={() => setModalState({ ...modalState, delete: false, selected: null })} title="Xác nhận Xóa" size="md">
             ...
             <Modal isOpen={modalState.bulkDelete} onClose={() => setModalState({ ...modalState, bulkDelete: false })} title="Xác nhận Xóa Hàng loạt" size="md">
                <p>Bạn có chắc chắn muốn xóa <strong className="text-light-text-main dark:text-dark-text-main">{selectedInstructors.length}</strong> giảng viên đã chọn? Hành động này không thể hoàn tác.</p>
                 <div className="flex justify-end space-x-4 pt-6">
                    <button type="button" onClick={() => setModalState({ ...modalState, bulkDelete: false })} className="bg-light-bg-nav dark:bg-dark-bg-nav hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover px-6 py-2 rounded-lg">Hủy</button>
                    <button type="button" onClick={handleBulkDelete} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg">Xóa</button>
                </div>
            </Modal>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
             ...
             <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                {selectedInstructors.length === 0 ? (
                    <>
                        <div className="relative w-full md:w-1/3">
                            ...
                            <input 
                                ...
                                className="w-full bg-light-bg-card dark:bg-dark-bg-card border border-light-border dark:border-dark-border rounded-lg pl-10 pr-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
                                value={searchTerm}
                                onChange={e => clearSelectionAndSetPage(setSearchTerm, e.target.value)}
                            />
                        </div>
                        <div className="flex flex-wrap items-center gap-2 justify-start md:justify-end w-full md:w-auto">
                            <select 
                                className="bg-light-bg-card dark:bg-dark-bg-card border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
                                value={statusFilter}
                                onChange={e => clearSelectionAndSetPage(setStatusFilter, e.target.value)}
                            >
                                <option value="all">Tất cả trạng thái</option>
                                <option value="Hoạt động">Hoạt động</option>
                                <option value="Không hoạt động">Không hoạt động</option>
                            ...
                            <select 
                                className="bg-light-bg-card dark:bg-dark-bg-card border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue"
                                value={majorFilter}
                                onChange={e => clearSelectionAndSetPage(setMajorFilter, e.target.value)}
                            >
                    ...
                    <div className="w-full flex justify-between items-center bg-primary-blue/10 dark:bg-primary-blue/20 p-2 rounded-lg">
                        <span className="font-semibold px-2">{selectedInstructors.length} giảng viên được chọn</span>
                        <button onClick={() => setModalState({...modalState, bulkDelete: true})} className="flex items-center bg-red-600 hover:bg-red-700 text-white font-semibold py-2 px-4 rounded-lg transition-colors">
                ...
                <table className="w-full text-left min-w-[800px]">
                    <thead className="bg-light-bg-nav dark:bg-dark-bg-nav">
                        <tr>
                            <th className="p-4 w-12">
                                <input
                                    type="checkbox"
                                    className="bg-light-bg-card dark:bg-dark-bg-card border-light-border dark:border-dark-border rounded focus:ring-primary-blue"
                                    checked={paginatedInstructors.length > 0 && selectedInstructors.length === paginatedInstructors.length}
                                    onChange={handleSelectAll}
                                    ref={el => { if (el) el.indeterminate = selectedInstructors.length > 0 && selectedInstructors.length < paginatedInstructors.length; }}
                                />
                            </th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Họ và Tên</th>
                    ...
                    <tbody>
                        {paginatedInstructors.map(instructor => (
                            <tr key={instructor.id} className={`border-b border-light-border dark:border-dark-border last:border-b-0 transition-colors ${selectedInstructors.includes(instructor.id) ? 'bg-primary-blue/10 dark:bg-primary-blue/20' : 'hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover'}`}>
                                <td className="p-4">
                                     <input
                                        type="checkbox"
                                        className="bg-light-bg-card dark:bg-dark-bg-card border-light-border dark:border-dark-border rounded focus:ring-primary-blue"
...
const CourseManagementPage: React.FC<{ onOpenSendModal: (data: Partial<NotificationLog>) => void }> = ({ onOpenSendModal }) => {
                    ...
                    <tbody>
                        {courses.map(course => (
                            <tr key={course.code} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                ...
                                <td className="p-4">
                                    <div className="flex items-center">
                                        <span>{course.students.enrolled} / {course.students.capacity}</span>
                                        {course.students.enrolled / course.students.capacity >= 0.9 && (
                                            <div title="Lớp học sắp đầy" className="ml-2">
                                                <TriangleAlertIcon className="w-5 h-5 text-status-yellow" />
                                            </div>
                                        )}
                                    </div>
                                </td>
                                ...
                                <td className="p-4">
                                     <div className="flex justify-center space-x-2 text-light-text-muted dark:text-dark-text-muted">
                                        <button onClick={() => onOpenSendModal({ recipient: `${course.name} (${course.code})`, type: 'Schedule' })} title="Nhắc lịch học" className="hover:text-primary-blue p-1 rounded-md hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav"><BellIcon className="w-5 h-5" /></button>
...
const FinancialManagementPage: React.FC<{ onOpenSendModal: (data: Partial<NotificationLog>) => void }> = ({ onOpenSendModal }) => {
                            ...
                            {paginatedTransactions.map(tx => (
                                <tr key={tx.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                    ...
                                    <td className="p-4">
                                     <div className="flex justify-center space-x-2 text-light-text-muted dark:text-dark-text-muted">
                                        {tx.type === 'Thu' && (
                                             <button onClick={() => onOpenSendModal({ recipient: tx.person, type: 'Tuition' })} title="Nhắc đóng tiền" className="hover:text-primary-blue p-1 rounded-md hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav"><BellIcon className="w-5 h-5" /></button>
                                        )}
...
const ExamScheduleManagementPage: React.FC<{ onOpenSendModal: (data: Partial<NotificationLog>) => void }> = ({ onOpenSendModal }) => {
                    ...
                    <tbody>
                        {paginatedExams.map(exam => (
                            <tr key={exam.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                ...
                                <td className="p-4">
                                     <div className="flex justify-center space-x-2 text-light-text-muted dark:text-dark-text-muted">
                                        <button onClick={() => onOpenSendModal({ recipient: `${exam.name} (Kỳ thi)`, type: 'Exam' })} title="Nhắc lịch thi" className="hover:text-primary-blue p-1 rounded-md hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav"><BellIcon className="w-5 h-5" /></button>
...
const TrainingFieldManagementPage: React.FC = () => {
    ...
    return (
        <div>
            ...
            <Modal isOpen={modalState.add || modalState.edit} onClose={() => setModalState({ ...modalState, add: false, edit: false, selected: null })} title={modalState.add ? "Thêm Lĩnh vực" : "Chỉnh sửa Lĩnh vực"}>
                <form onSubmit={modalState.add ? handleAddField : handleUpdateField} className="space-y-4">
                    <div>
                        <label className="block text-sm font-medium mb-2">Tên lĩnh vực</label>
                        <input name="name" type="text" required defaultValue={modalState.selected?.name} className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue" />
                    </div>
                     <div>
                        <label className="block text-sm font-medium mb-2">Mô tả</label>
                        <textarea name="description" required defaultValue={modalState.selected?.description} className="w-full bg-light-bg-main dark:bg-dark-bg-main border border-light-border dark:border-dark-border rounded-lg px-4 py-2 focus:outline-none focus:ring-2 focus:ring-primary-blue h-24" />
                    </div>
                    <div className="flex justify-end space-x-4 pt-4">
                        <button type="button" onClick={() => setModalState({ ...modalState, add: false, edit: false, selected: null })} className="bg-light-bg-nav dark:bg-dark-bg-nav hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover px-6 py-2 rounded-lg">Hủy</button>
                        <button type="submit" className="bg-primary-blue hover:bg-primary-blue-hover text-white px-6 py-2 rounded-lg">Lưu</button>
            ...
            <Modal isOpen={modalState.delete} onClose={() => setModalState({ ...modalState, delete: false, selected: null })} title="Xác nhận Xóa" size="md">
                <p>Bạn có chắc chắn muốn xóa lĩnh vực <strong className="text-light-text-main dark:text-dark-text-main">{modalState.selected?.name}</strong>?</p>
                 <div className="flex justify-end space-x-4 pt-6">
                    <button type="button" onClick={() => setModalState({ ...modalState, delete: false, selected: null })} className="bg-light-bg-nav dark:bg-dark-bg-nav hover:bg-gray-200 dark:hover:bg-dark-bg-nav-hover px-6 py-2 rounded-lg">Hủy</button>
                    <button type="button" onClick={handleDeleteField} className="bg-red-600 hover:bg-red-700 text-white px-6 py-2 rounded-lg">Xóa</button>
                </div>
            </Modal>
            ...
            <div className="bg-light-bg-card dark:bg-dark-bg-card rounded-lg overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-light-bg-nav dark:bg-dark-bg-nav">
                        <tr>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">ID</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Tên lĩnh vực</th>
                            <th className="p-4 text-sm font-semibold text-light-text-muted dark:text-dark-text-muted">Mô tả</th>
                    ...
                    <tbody>
                        {paginatedFields.map(field => (
                            <tr key={field.id} className="border-b border-light-border dark:border-dark-border last:border-b-0 hover:bg-light-bg-nav dark:hover:bg-dark-bg-nav-hover">
                                <td className="p-4 text-sm text-light-text-muted dark:text-dark-text-muted">#{field.id}</td>
...
const CandidateManagementPage: React.FC = () => {
    ...
    // ... simplified logic for candidates, similar to others
    return <div><h1 className="text-3xl font-bold">Quản lý Thí sinh</h1><p>Tính năng đang phát triển.</p></div>;
};

const ReportsPage: React.FC = () => {
    return <div><h1 className="text-3xl font-bold">Báo cáo</h1><p>Tính năng đang phát triển.</p></div>;
};

const ProfilePage: React.FC<{ profile: AdminProfile }> = ({ profile }) => {
    return (
        <div className="max-w-2xl mx-auto bg-light-bg-card dark:bg-dark-bg-card p-8 rounded-lg shadow">
            <div className="flex flex-col items-center">
                <img src={profile.avatar} alt={profile.name} className="w-32 h-32 rounded-full mb-4 object-cover" />
                <h2 className="text-2xl font-bold">{profile.name}</h2>
                <p className="text-light-text-muted dark:text-dark-text-muted">{profile.role}</p>
            </div>
            <div className="mt-8 space-y-4">
                <div><label className="font-medium">Email:</label> <span className="ml-2">{profile.email}</span></div>
                <div><label className="font-medium">Phone:</label> <span className="ml-2">{profile.phone}</span></div>
            </div>
        </div>
    );
};
...
const SettingsPage: React.FC = () => {
    return <div><h1 className="text-3xl font-bold">Cài đặt</h1><p>Cài đặt hệ thống.</p></div>;
};
...
const App: React.FC = () => {
    ...
    const [sendModalData, setSendModalData] = useState<Partial<NotificationLog>>({});
    ...
    const openSendModal = (data: Partial<NotificationLog> = {}) => {
        setSendModalData(data);
        setSendModalOpen(true);
    };

    const handleSendNotification = (newNotification: NotificationLog) => {
        setNotifications([newNotification, ...notifications]);
    };

    const renderPage = () => {
        switch (activePage) {
            case 'Dashboard': return <DashboardPage theme={theme} notifications={notifications} />;
            ...
            case 'StaffManagement': return <StaffManagementPage />;
            case 'ExamScheduleManagement': return <ExamScheduleManagementPage onOpenSendModal={openSendModal} />;
            case 'ClassroomManagement': return <ClassroomManagementPage />;
            case 'TrainingFieldManagement': return <TrainingFieldManagementPage />;
            case 'CandidateManagement': return <CandidateManagementPage />;
            case 'Reports': return <ReportsPage />;
            case 'Profile': return <ProfilePage profile={adminProfile} />;
            case 'Settings': return <SettingsPage />;
            default: return <DashboardPage theme={theme} notifications={notifications} />;
        }
    };
